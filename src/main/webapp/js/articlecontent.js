...
function showCommentsData(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        //拼接表格的行和列
        str = "<div><div>" + data[i].comment_author_id + "</div><div>" + data[i].comment_content + "</div><div>" + data[i].comment_created + "</div><div><button id=\""+"f"+data[i].comment_id+"f"+data[i].comment_author_id +"\" onclick=\"show_reply_input(this.id" +
            ...
            "    </div></div> </div><div><div><div id=\"show"+data[i].comment_id+"\" style='display: none'><table id=\"reply_tab"+data[i].comment_id+"\"></table><button id=\"v"+data[i].comment_id+"\" onclick=\"hidecomments_reply(this.id)\">收起</button></div></div></div> ";//追加到table中
...
function create_commnets() {
    var comment_content=$('#create_comment').text();
    var href_info=location.href;
    var article_id=href_info.split("=")[1];
    ...
    var read_info = "false";
    $.get("http://localhost:8090/Create_comments?comment_content="+comment_content+"&article_id=" + article_id+"&comment_author_id=" + author_id+"&comment_read="+read_info,function (data) {
        console.log(data);
