...
function get_random_book() {
    ...
    for(var i=1;i<=6;i++){
        ...
        $.get("http://localhost:8090/getByNum?num=" + i, function (data) {
            str_ = "<a href=\"book_info.html?book_id="+data.num+"\"><div class='book_info_div'><div>" + data.num + "</div><div>" + data.book_name + "</div><div>" + data.price + "</div></div></a>";//追加到table中
...
function showData(data) {
    ...
    for (var i = (book_page-1)*6; i <book_page*6 ; i++) {
        //拼接表格的行和列
        str = "<a href=\"book_info.html?book_id="+data[i].num+"\"><div class='book_info_div'><div>" + data[i].num + "</div><div>" + data[i].book_name + "</div><div>" + data[i].price + "</div></div></a>";//追加到table中
...
function search_button_action() {
    ...
    var search_input = document.getElementById("search_book_input")
    search_input.value = key_word;
    get_Book();
}
function get_Book(){
